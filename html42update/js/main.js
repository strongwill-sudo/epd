...
async function sendclass() {
  const classInput = document.getElementById('cmdCLASS').value;
  if (!classInput) {
    alert('请输入课程表数据');
    return;
  }
  ...
  // 验证输入格式并解析课程表
  const classData = classInput.split(',');
  if (classData.length !== 5) {
    alert('请输入5天的课程数据，用逗号分隔');
    return;
  }

  // 验证每天的课程数据格式
  for (let i = 0; i < classData.length; i++) {
    const dayData = classData[i].trim();
    // 检查是否以正确的星期数开头 (1-7)
    if (dayData.length < 1 || parseInt(dayData[0]) !== i + 1) {
      alert(`第${i + 1}天课程数据格式错误，应以${i + 1}开头`);
      return;
    }
    // 检查后续课程代码是否为偶数位（每两位代表一个课程）
    if ((dayData.length - 1) % 2 !== 0) {
      alert(`第${i + 1}天课程数据格式错误，课程代码应为偶数位`);
      return;
    }
  }
  ...
  try {
    ...
    // 发送课程表命令和数据
    const bytes = hex2bytes(hexData);
    if (await write(EpdCmd.SEND_CLASS, bytes)) {
      addLog("课程表已发送成功！");
    }
  } catch (e) {
    console.error("发送课程表失败:", e);
    addLog("发送课程表失败: " + e.message);
  }
}